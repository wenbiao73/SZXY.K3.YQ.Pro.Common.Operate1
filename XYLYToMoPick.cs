...
{
    [HotUpdate]
    [Description("流延到生产领料单单据转换")]
    public class XYLYToMoPick : AbstractConvertPlugIn
    {
                    ...
                    if (EntryCollect != null && EntryCollect.Count > 0)
                    {
                        foreach (DynamicObject Row in EntryCollect)
                        {
                            ...
                            if (MoId != "0")
                            {
                                ...
                                if (MoObejct != null && !Convert.ToString(Row["MoEntrySeq"]).IsNullOrEmptyOrWhiteSpace() && !FMATERIALID.IsNullOrEmptyOrWhiteSpace()&& FMATERIALID!="")
                                {
